...
class ResponseInitiative{
    ...
    protected static $queryUrl2 = 'https://api.weixin.qq.com/cgi-bin/message/template/send?access_token=';
    ...
    public static function text($tousername, $content){
        ...
        $template=self::json_encode_ex($template);
...
//发送模板消息
    public static function send_template($tousername,$array){
        //获取ACCESS_TOKEN
        $accessToken = AccessToken::getAccessToken();
        ...
        $template = array(
            'touser'=>$tousername,
            'template_id'=>'B73velgxMjCqjLp2vM1bsPPJ0QgDIiOYSd9pJELmmC4',
            'url'=>'http://liuliangbao.sevenkick.com/index.php/UserCenter/index.html',
            'data'=>$array,
        );
        $template = json_encode($template);
        return Curl::callWebServer(self::$queryUrl2.$accessToken, $template, self::$action);
    }

    //发送模板消息
    public static function send_template_by_tid($tousername,$array,$template_id,$url=''){
       $urlindex=$url==''?'http://liuliangbao.sevenkick.com/index.php/UserCenter/user_coupon.html':$url;
        //获取ACCESS_TOKEN
        $accessToken = AccessToken::getAccessToken();
